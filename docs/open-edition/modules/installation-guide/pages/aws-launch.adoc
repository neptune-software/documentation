...
//Fabian: "panel"? In the previous section it was called "section". I dont know if it is already in the style guide.
. On the *Launch instance* panel, click *Launch instance* and select *Launch instance* from the drop-down menu.
//Helle@parson: Sadly, both the panel, button, and drop-down have the same name, see screenshot.
//Fabian: Maybe show the "Launch instance" in the drop down menu as well to show that the same phrase is used three times here.
+
...
//Fabian: Steps start with "1" again here, was that intentional?
//Fabian: Are there better names than "first drop-down menu" or "second drop-down menu"?
...
//Fabian: Avoid phrases like "a lot of".
The panel shows a lot of information, including the name, ID, and status of your instance.
...
//Fabian: Write "SSH client" with small letters like in the screenshot.
//Fabian: Again, counting restarts at "1", was that intentional?
