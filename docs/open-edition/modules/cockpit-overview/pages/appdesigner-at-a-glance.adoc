...
//Fabian: It is a bit confusing that the order in the list is different to the order in the screenshot. As a reader I would normally expect the list and figure to be aligned.
...
//Fabian: It is Ok to use them within one topic but never use them to link to subchapters in other topics. To do that, provide a unique UUID to a subtopic as anchor (write uuid in square brackets above the chapter).

image::../../cockpit-overview/images/appdesigner-interface-clear.png[width=800]
//Helle@parson: new SUI, a bit clearer and in standard view. GUI changed (compare reusable component panels). If okay, other screenshots tbd.
//Fabian: Looks good. Do we know this is the final version? It would not be good if we have to change it again in a few weeks.
//Fabian: There are no yellow boxes for "Open UI5" and "Neptune Community" in the bottom left corner. Did you leave them out on purpose?
