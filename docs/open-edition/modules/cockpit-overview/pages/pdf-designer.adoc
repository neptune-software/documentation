...
//Fabian: Settings tab need to be described in an extra topic
+
[TIP]
====
To configure the settings, you might need to click the *toggle to edit* icon.
//Fabian: Do not use "might". Explain, when it is necessary to press this button. See the TIP in https://docs-review.neptune-software.com/neptune-dxp-open-edition/21/cockpit-overview/script-editor-codesnippets.html
...
//"Header", "Footer" etc. entweder Fett und groß (also GUI-Element), oder nicht ausgezeichnet und klein geschrieben
...
//"Header", "Footer" etc. entweder Fett und groß (also GUI-Element), oder nicht ausgezeichnet und klein geschrieben. Do not use simple, describe better what you mean, for example, "content related"
...
//No, do not have it twice in the same topic
...
//Divide this section into groups and give them subheadings -> DONE
