//Fabian@Helle: After looking through this topic, I would split it up in three task topics. Now it is too long. It would also give you the opportunity to add some more context information in the beginning of a topic (for example, what is an API, what is an "operation"
...
//Fabian@Helle: Das "Neptune DXP - Open Edition" weglassen hier, ist überflüssig. Außerdem "Cockpit" immer groß schreiben.
...
//Maybe add a screenshot here of the "operations" tab?
. In *Path*, enter a slash and the name of your table definition, for example, `/clonetroopers`.
//Fabian@Helle: Again, why give an example here at all? (And if so, maybe a more generic one, for example "/neptune"? Star Wars is nice, but it might distract user from the actual task)
